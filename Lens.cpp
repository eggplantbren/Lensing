...
void Lens::alphaGrid(const MeshGrid& grid, Array<double>& ax, Array<double>& ay) const
{
...
Array<double> Lens::densityGrid(const MeshGrid& grid, double h) const
{
	Array<double> results(grid.get_ni(), grid.get_nj());
	for(int i=0; i<grid.get_ni(); i++)
		for(int j=0; j<grid.get_nj(); j++)
